...
public class LanguageCraftAPI {
	...
	public LanguageCraftAPI(String externPlugin) {
		this.plugin = (MultiLanguage) Bukkit.getServer().getPluginManager().getPlugin("LanguageCraft");
	...
	public void addLanguage(Language language, InputStream resource, boolean defaultLang) {
		File langFile = new File(localisationDirectory, language.getFullName() + ".yml");
		...
		languages.put(language.getFullName().toLowerCase(), langFile);
